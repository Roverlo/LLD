...
const createIpPlanWorksheet = (
    ...
    // 从参数中获取绑定模式
    const { mngBondMode, bizBondMode, storBondMode } = params;
...
const createServerPerformanceWorksheet = (workbook, servers, params) => {
    const worksheet = workbook.addWorksheet('服务器性能评估');
    ...
    // 从参数中获取服务器配置信息
    const mgmtConfig = {
        cpuCores: params.mgmtCpuCores || 0,
        memory: params.mgmtMemory || 0,
        ssd: params.mgmtSsdCount || 0,
        ssdSpec: params.mgmtSsdSpec || '',
        hdd: params.mgmtHddCount || 0,
        hddSpec: params.mgmtHddSpec || ''
    };

    const compConfig = {
        cpuCores: params.compCpuCores || 0,
        memory: params.compMemory || 0,
        ssd: params.compSsdCount || 0,
        ssdSpec: params.compSsdSpec || '',
        osd: params.compOsdCount || 0,
        osdSpec: params.compOsdSpec || ''
    };

    const storConfig = {
        cpuCores: params.storCpuCores || 0,
        memory: params.storMemory || 0,
        ssd: params.storSsdCount || 0,
        ssdSpec: params.storSsdSpec || '',
        osd: params.storOsdCount || 0,
        osdSpec: params.storOsdSpec || '',
        hdd: params.storHddCount || 0,
        hddSpec: params.storHddSpec || ''
    };
    ...
    const serverTypes = [
        {
            type: '管理服务器',
            count: serverStats['管理服务器']?.count || 0,
            config: mgmtConfig,
            performance: '负责集群管理、监控等功能'
        },
        {
            type: '计算（超融合）服务器',
            count: serverStats['计算（超融合）服务器']?.count || 0,
            config: compConfig,
            performance: '提供计算资源和存储服务'
        },
        {
            type: '存储服务器',
            count: serverStats['存储服务器']?.count || 0,
            config: storConfig,
    ...
    serverTypes.forEach(serverType => {
        if (serverType.count > 0) {
            ...
            if (config.osd > 0) {
                ...
                storageConfig += `OSD: ${config.osd}块×${config.osdSpec}TB`;
            }
            if (config.hdd > 0) {
                if (storageConfig) storageConfig += ', ';
...
const createServerWorksheet = (workbook, servers, params) => {
    ...
    // 设置标题行
    const headers = ['主机名', '角色', '管理网级联端口名', '管理网级联网卡', '管理网IP', '业务网级联端口名', '业务网级联网卡', '业务网IP', '存储网级联端口名', '存储网级联网卡', '存储公共网IP', '存储集群网IP'];
    worksheet.addRow(headers);
    ...
    servers.forEach((server, index) => {
        ...
        worksheet.addRow([
            server.hostname,
            server.role,
            isFloatingIp ? '不涉及' : params.mngBondName,
            isFloatingIp ? '不涉及' : params.mngBondNics,
            server.mngIp,
            isFloatingIp ? '不涉及' : params.bizBondName,
            isFloatingIp ? '不涉及' : params.bizBondNics,
            server.bizIp,
            isFloatingIp ? '不涉及' : params.storBondName,
            isFloatingIp ? '不涉及' : params.storBondNics,
            server.pubIp,
            server.cluIp,
        ]);
        ...
        if (server.role.includes('浮动IP') || server.role.includes('浮动')) {
            ...
            commentCell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
        }

        // 为"待提供IP"单元格添加蓝色背景
        const isFloatingIpForIpCheck = server.isFloatingIp || server.role.includes('浮动IP') || server.role.includes('浮动');
        const ipColumns = {
            '管理网IP': server.mngIp,
            '业务网IP': server.bizIp,
...
const generateExcelFile = async (plan, mainWindow) => {
    try {
        ...
        createServerPerformanceWorksheet(workbook, plan.servers, plan.metadata.parameters);
        createServerWorksheet(workbook, plan.servers, plan.metadata.parameters);
